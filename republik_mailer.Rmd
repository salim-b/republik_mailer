                                        ...
                                        auth_cookie = config$auth_cookie)
{
  ...
  #     } else
  #     {
  #       rlang::abort(message = glue::glue("Curl repeatedly failed to fetch content from <https://www.republik.ch/format/{format}>! Last exit status code was ",
...
update_article_metadata <- function(formats = c("alles-was-recht-ist",
                                                "am-gericht",
                                                "briefing-aus-bern",
                                                "preis-der-republik"))
{
...
- These are simple wrapper functions around the CLI tool [minify](https://github.com/tdewolff/minify/tree/master/cmd/minify). Use parameter `command` to specify where its executable is found on your system (just leave it at its default `command = "minify"` if it's in the system's `$PATH`).

- Precompiled versions of minify can be downloaded [here](https://github.com/tdewolff/minify/releases). If you want or have to compile it yourself (e.g. for 32-bit plattforms), make sure the [Go distribution](https://golang.org/doc/install) and [Git](https://git-scm.com/downloads) are installed and run the following:
...
- Beforehand you should [install mailsend-go](https://rich-iannone.github.io/blastula/articles/sending_using_smtp.html#installation-of-mailsend-go).
    ...
    On Debian/Ubuntu Linux you also have to install the Debian package `libsodium-dev` which is required by the R package `sodium`, which in turn is a dependency of the R package `keyring`, which is finally a dependency of the `blastula` R package.

- It's recommended to store the SMTP credentials to file. See the [README](README.md#-config) for details.

- It's recommended to assign the class `img-responsive` to images embedded in the mail text. Currently this _cannot_ be achieved by using Markdown (`![Alternative Text...](https://...)`). Instead you have to use plain HTML:
