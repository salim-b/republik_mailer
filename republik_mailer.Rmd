...
An alternative way to fetch the HTML using the external curl CLI would look like[^curl-syscmd]:
...
[^curl-syscmd]: Note that the [cURL](https://curl.haxx.se/) command line tool must be available on the system for this to work.
...
update_article_metadata <- function(formats = config$default_formats) {
  ...
  article_metadata %>% readr::write_rds(path = "article_metadata.rds")
}
```
